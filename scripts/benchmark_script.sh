...
if [ $diff_time -gt $seconds_to_wait ]
then
   echo $diff_time > $outputpath/tmp
   echo $(date +%s) > $outputpath/last_run

# What is the load average?
# ------------------------------------------------------------------------

read LOAD < /proc/loadavg
#echo "load = " $LOAD >> /tmp/diagnose/$1
...
#echo "num_users = " $NUM_USERS >> /tmp/diagnose/$1
...
echo "[Benchmarks]" >> $outputpath/$outputfile
echo "name           = $0" >> $outputpath/$outputfile
echo "stream_copy    = " $COPY >> $outputpath/$outputfile
echo "stream_scale   = " $SCALE >> $outputpath/$outputfile
echo "stream_add     = " $ADD >> $outputpath/$outputfile
echo "stream_trial   = " $TRIAD >> $outputpath/$outputfile
...
echo "linpack_5k     = " $V1 >> $outputpath/$outputfile
echo "linpack_10k    = " $V2 >> $outputpath/$outputfile
